...
文山采用 **证据门槛式纵向框架分析**（Evidence-Gated Longitudinal Framework Analysis，EGLFA）：这是文山组合 Framework Method、定性内容分析、扎根理论编码、纵向主题分析与论证挖掘形成的方法规格，不是已经存在的论文方法名。
...
| `35篇` | 35 篇独立、有效、归并后的文章，也是这座山的海拔；代表积累量，不代表知识水平 |
...
4. **山界与稳定性审查**：检查超大主题、子主题、回答覆盖率，并对同一语料重复分析。
5. **确定性渲染**：以独立文章数作为海拔，生成带时间戳的中英双语等高线地图和传播截图。
