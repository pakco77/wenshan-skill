...
.obsidian-atlas .tools{background:#171814;border-color:#665d50;color:#e6dac3;box-shadow:0 18px 38px #26231f4d}.obsidian-atlas .control-head{color:#8f8372;border-color:#4c463d}.obsidian-atlas .tool{color:#ded3bd;border-color:#413c34}.obsidian-atlas .tool b,.obsidian-atlas .tool small{color:#8f8372}.obsidian-atlas .tool:hover{background:#24231e;color:#fff8ea;box-shadow:inset 2px 0 #a99778}
...
let terrainCache=null;function paintGrid(){if(theme!=='obsidian-atlas')return;const p=palette(),step=60;ctx.save();ctx.strokeStyle=p.grid;ctx.lineWidth=.7;ctx.globalAlpha=.10;for(let x=0;x<=W;x+=step){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,H);ctx.stroke()}for(let y=0;y<=H;y+=step){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke()}ctx.globalAlpha=.16;ctx.lineWidth=.8;for(let y=step;y<H;y+=step*2)for(let x=step;x<W;x+=step*2){ctx.beginPath();ctx.moveTo(x-4,y);ctx.lineTo(x+4,y);ctx.moveTo(x,y-4);ctx.lineTo(x,y+4);ctx.stroke()}ctx.restore()}
function paintDust(){if(theme!=='obsidian-atlas')return;const p=palette();ctx.save();ctx.fillStyle=p.dust;for(let i=0;i<150;i++){const x=seeded(`dust-x-${i}`,i)*W,y=seeded(`dust-y-${i}`,i+91)*H,r=.45+seeded(`dust-r-${i}`,i+181)*.9;ctx.globalAlpha=.07+seeded(`dust-a-${i}`,i+271)*.14;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()}ctx.restore()}
function paintRange(){const field=terrainCache||(terrainCache=terrainField()),rings=theme==='obsidian-atlas'?15:13,p=palette();ctx.save();ctx.strokeStyle=p.line;ctx.lineCap='round';ctx.lineJoin='round';for(let ring=1;ring<=rings;ring++){const t=ring/rings,level=field.max*(.040+Math.pow(t,1.45)*.82),major=ring===1||ring===rings||ring%3===0;ctx.globalAlpha=theme==='obsidian-atlas'?(major?.46:.20):(major?.58:.28);ctx.lineWidth=(major?(theme==='obsidian-atlas'?1.65:1.75):.78)/view.scale;for(const chain of connectSegments(levelSegments(field,level)))strokeSmooth(chain,field.nx,field.ny)}ctx.restore()}
...
function peripheralLabels(h,x,y,selected){if(theme!=='obsidian-atlas')return;const terms=(h.subpeaks||[]).slice(0,3),p=palette(),offsets=[[-110,-64],[112,-38],[-92,104]];ctx.save();ctx.textAlign='center';ctx.fillStyle=p.muted;ctx.strokeStyle=p.line;ctx.globalAlpha=selected ? .72 : .38;ctx.font='500 12px -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif';terms.forEach((term,index)=>{const text=language==='en'?(term.label_en||term.label):term.label,[dx,dy]=offsets[index];if(!text)return;ctx.fillText(text,x+dx,y+dy);const width=Math.min(56,Math.max(18,ctx.measureText(text).width*.58));ctx.lineWidth=.7/view.scale;ctx.beginPath();ctx.moveTo(x+dx-width/2,y+dy+7);ctx.lineTo(x+dx+width/2,y+dy+7);ctx.stroke()});ctx.restore()}
function label(h){const x=h.x*W,y=h.y*H,p=palette(),words=q(),text=copy(h),selected=active?.id===h.id,dense=hills.length>=7,titleWidth=dense?215:330;ctx.save();ctx.textAlign='center';ctx.globalAlpha=active&&!selected ? .55 : 1;ctx.fillStyle=p.dot;ctx.beginPath();ctx.moveTo(x,y-34);ctx.lineTo(x-10,y-10);ctx.lineTo(x+10,y-10);ctx.closePath();ctx.fill();const size=titleFont(text.label,titleWidth),titleSize=selected?Math.min(size+3,30):dense?Math.min(size,25):size;ctx.fillStyle=p.title;ctx.font=`680 ${titleSize}px -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif`;ctx.strokeStyle=p.bg;ctx.lineWidth=(theme==='obsidian-atlas'?4:5)/view.scale;ctx.strokeText(text.label,x,y+25);ctx.fillText(text.label,x,y+25);ctx.fillStyle=p.count;ctx.font=`780 ${selected?26:dense?23:24}px ui-monospace,SFMono-Regular,Menlo,monospace`;ctx.strokeStyle=p.bg;ctx.lineWidth=3.8/view.scale;ctx.strokeText(words.pieces(h.count),x,y+57);ctx.fillText(words.pieces(h.count),x,y+57);peripheralLabels(h,x,y,selected);if(!dense&&theme!=='obsidian-atlas'){ctx.fillStyle=p.muted;ctx.font='12px -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif';wrap(text.answer,x,y+83,310,19)}ctx.restore()}
...
function draw(){const p=palette();ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle=p.bg;ctx.fillRect(0,0,W,H);paintGrid();ctx.save();ctx.translate(view.x,view.y);ctx.scale(view.scale,view.scale);paintDust();paintRange();for(const hill of hills){ctx.save();for(const point of hill.points){ctx.beginPath();ctx.arc(point.x*W,point.y*H,(theme==='obsidian-atlas'?2.45:3.1)/view.scale,0,Math.PI*2);ctx.fillStyle=p.dot;ctx.globalAlpha=active&&active.id!==hill.id?(theme==='obsidian-atlas'?.22:.38):(theme==='obsidian-atlas'?.48:.76);ctx.fill();ctx.strokeStyle=p.dotStroke;ctx.globalAlpha=active&&active.id!==hill.id?(theme==='obsidian-atlas'?.22:.48):(theme==='obsidian-atlas'?.62:.92);ctx.lineWidth=(theme==='obsidian-atlas'?.7:1.15)/view.scale;ctx.stroke()}ctx.restore()}if(active)focusRing(active);for(const hill of hills.filter(h=>h.id!==active?.id))label(hill);if(active)label(active);ctx.restore()}
