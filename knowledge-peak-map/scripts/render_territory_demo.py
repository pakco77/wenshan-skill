...
const MONO={rgb:[72,75,71],line:'#4c504c',dot:'#30332f'},COLORS={};DATA.territories.forEach(territory=>COLORS[territory.id]=MONO);
...
let terrainCache=null;function paintRange(){const field=terrainCache||(terrainCache=terrainField()),rings=13;ctx.save();ctx.strokeStyle=MONO.line;ctx.lineCap='round';ctx.lineJoin='round';for(let ring=1;ring<=rings;ring++){const t=ring/rings,level=field.max*(.040+Math.pow(t,1.45)*.82),major=ring===1||ring===rings||ring%3===0;ctx.globalAlpha=major?.58:.28;ctx.lineWidth=(major?1.75:.82)/view.scale;for(const chain of connectSegments(levelSegments(field,level)))strokeSmooth(chain,field.nx,field.ny)}ctx.restore()}
...
function label(h){const x=h.x*W,y=h.y*H,c=COLORS[h.id],words=q(),text=copy(h),selected=active?.id===h.id,dense=hills.length>=7,titleWidth=dense?200:330;ctx.save();ctx.textAlign='center';ctx.globalAlpha=active&&!selected ? .64 : 1;ctx.fillStyle=c.line;ctx.font=`650 ${selected?20:dense?17:22}px -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif`;ctx.fillText(words.pieces(h.count),x,y-28);ctx.fillStyle=c.dot;ctx.beginPath();ctx.moveTo(x,y-(selected?23:20));ctx.lineTo(x-(selected?12:10),y+6);ctx.lineTo(x+(selected?12:10),y+6);ctx.closePath();ctx.fill();ctx.fillStyle='#242722';const size=titleFont(text.label,titleWidth);ctx.font=`650 ${selected?Math.min(size+3,29):dense?Math.min(size,24):size}px -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif`;ctx.strokeStyle='#e8e5dc';ctx.lineWidth=5/view.scale;ctx.strokeText(text.label,x,y+42);ctx.fillText(text.label,x,y+42);if(!dense){ctx.fillStyle='#575a53';ctx.font='12px -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif';wrap(text.answer,x,y+71,310,19)}ctx.restore()}
function focusRing(hill){const x=hill.x*W,y=hill.y*H,r=Math.max(112,...hill.points.map(p=>Math.hypot(p.x*W-x,p.y*H-y)+32));ctx.save();ctx.strokeStyle='#2f3430';ctx.globalAlpha=.78;ctx.lineWidth=2.4/view.scale;ctx.setLineDash([8/view.scale,5/view.scale]);ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.globalAlpha=.18;ctx.lineWidth=14/view.scale;ctx.beginPath();ctx.arc(x,y,r+2,0,Math.PI*2);ctx.stroke();ctx.restore()}
function draw(){ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle='#e8e5dc';ctx.fillRect(0,0,W,H);ctx.save();ctx.translate(view.x,view.y);ctx.scale(view.scale,view.scale);paintRange();for(const hill of hills){ctx.save();ctx.globalAlpha=active&&active.id!==hill.id ? .44 : 1;for(const p of hill.points){ctx.beginPath();ctx.arc(p.x*W,p.y*H,3.1/view.scale,0,Math.PI*2);ctx.fillStyle=COLORS[hill.id].dot;ctx.globalAlpha=active&&active.id!==hill.id ? .38 : .76;ctx.fill();ctx.strokeStyle='#e8e5dc';ctx.globalAlpha=active&&active.id!==hill.id ? .48 : .92;ctx.lineWidth=1.15/view.scale;ctx.stroke()}ctx.restore()}if(active)focusRing(active);for(const hill of hills.filter(h=>h.id!==active?.id))label(hill);if(active)label(active);ctx.restore()}
